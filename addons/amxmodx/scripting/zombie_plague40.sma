...
public plugin_init()
{
	...
	RegisterHam(Ham_Killed, "player", "fw_PlayerKilled")
 	...
 	register_event("HLTV", "EVENT_round_start2", "a", "1=0", "2=0")
...
public give_ammo_e (id, ammo){
	...
	new user_ammo = native_get_user_ammo_packs(id) + ammo;
	
	
	native_set_user_ammo_packs ( id, max ( 1, user_ammo) );
...
switch(tempid){
	case 33:
		{
			cur_add_amount = 100;
			client_print(id, print_chat, "/g[AP] /y%L /t%i /y%L", id, "CHANGED", cur_add_amount, id, "AMMO");
			menu_destroy(menu);
			menu_init(id);
	...
	default:
		{
			...
			client_print(id, print_chat, "/g[AP] /t%s /y%L /t%i /t%L!", Nick, id, "WAS_G", cur_add_amount, id, "AMMO")
			} else {
			menu_destroy(menu);
			menu_init(id);
			}
...
public fwd_Touch(toucher, touched)
{
	...
	native_set_user_ammo_packs(toucher, native_get_user_ammo_packs(toucher) + get_pcvar_num(pcvar[giveammo]))
...
public ShowHUD(taskid)
{
	...
	else
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "Zombie Plague 4.3^n %L: %d^n %L %s^n %L %d", id, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[ID_SHOWHUD])
	}
}
